      ...
      style={{
        ...
        <div data-ev-id="ev_2182c2495b" className="flex items-center justify-between px-4 py-2.5 border-b border-white/[0.06]">
          <div data-ev-id="ev_53f78eea76" className="flex items-center gap-3">
            ...
            <span data-ev-id="ev_a2e7dd7fab" className="text-white/20 text-[10px] font-mono">/ dashboard / overview</span>
          </div>
          <div data-ev-id="ev_cf403357ca" className="flex items-center gap-3">
            ...
            <div data-ev-id="ev_97290da914" className="relative">
              ...
              className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-white/[0.04] border border-white/[0.08] text-white/50 text-[11px] font-mono hover:bg-white/[0.08] transition-colors">
              ...
              <div data-ev-id="ev_7226dfe058" className="absolute top-full right-0 mt-1 bg-[#161b22] border border-white/[0.08] rounded-lg shadow-xl z-30 py-1 min-w-[100px]">
                ...
                r === timeRange ? 'text-cyan-400 bg-cyan-400/10' : 'text-white/50 hover:text-white/70 hover:bg-white/[0.04]'}`
                }>
            ...
            <button data-ev-id="ev_db026b1aa6" className="text-white/30 hover:text-white/60 transition-colors">
              <RefreshCw className={`w-3.5 h-3.5 ${tick % 2 === 0 ? 'animate-spin' : ''}`} style={{ animationDuration: '1s' }} />
            </button>
            <div data-ev-id="ev_6bf945cc3a" className="flex items-center gap-1.5 text-[10px] font-mono text-white/30">
        ...
        <div data-ev-id="ev_7973e4c9ca" className="grid grid-cols-1 sm:grid-cols-3 gap-3 px-4 pb-4">
          ...
          <div data-ev-id="ev_05903f7a99" className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-3">
            <div data-ev-id="ev_dd2a5b534a" className="flex items-center justify-between mb-3">
              <span data-ev-id="ev_75b4dc390e" className="text-white/50 text-[11px] font-mono">■ STATUS BREAKDOWN</span>
            ...
            <div data-ev-id="ev_a96059af17" className="flex items-center gap-4">
              ...
              <div data-ev-id="ev_42d7384f4b" className="flex flex-col gap-1.5">
                ...
                <div data-ev-id="ev_0fed395077" key={s.label} className="flex items-center gap-2 text-[10px]">
                    ...
                    <span data-ev-id="ev_d38d724140" className="text-white/50">{s.label}</span>
          ...
          <div data-ev-id="ev_e7add41a99" className="sm:col-span-2 rounded-lg border border-white/[0.06] bg-white/[0.02] p-3">
            <div data-ev-id="ev_c6da1162a1" className="flex items-center justify-between mb-3">
              <span data-ev-id="ev_263c9194b4" className="text-white/50 text-[11px] font-mono">■ SECTION ACTIVITY</span>
              <span data-ev-id="ev_7273dcd21e" className="text-white/20 text-[9px] font-mono">{timeRange}</span>
            </div>
                ...
                return (
                  <div data-ev-id="ev_91f7782b41" key={section.id} className="flex items-center gap-2.5">
                    <div data-ev-id="ev_fcacca7e39" className="flex-1 min-w-0">
                      ...
                      <div data-ev-id="ev_34eead5a09" className="text-white/30 text-[9px] font-mono">{section.links.length} services</div>
                    </div>
        ...
        {/* Footer */}
        <div data-ev-id="ev_7f83b1b679" className="flex items-center justify-between px-4 py-2 border-t border-white/[0.04] text-[9px] font-mono text-white/20">
...
const SummaryCard = ({ label, value, color, change, revealed, delay
  ...
  return (
    ...
    show ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'}`
      ...
      <div data-ev-id="ev_5e639615e7" className="text-white/40 text-[10px] font-mono mb-1.5">{label}</div>
  ...
  return (
    ...
    style={{ background: 'rgba(255,255,255,0.015)' }}>
      ...
      className="w-full flex items-center gap-2.5 px-3 py-2 border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
        ...
        <span data-ev-id="ev_8c1abf161e" className="text-white/25 text-[10px] font-mono">[{section.links.length}]</span>
        <Activity className="w-3 h-3 mr-auto" style={{ color: color + '60' }} />
        <span data-ev-id="ev_240d8efd12" className="text-white/20 text-[9px] font-mono">{section.links.filter((l) => fakeStatus(l.id) === 'healthy').length}/{section.links.length} UP</span>
        <ChevronDown className={`w-3.5 h-3.5 text-white/20 transition-transform ${expanded ? '' : '-rotate-90'}`} />
      </button>
          ...
          return (
            ...
            className="flex items-center gap-3 px-3 py-2 hover:bg-white/[0.03] transition-colors group focus:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded">
                ...
                <div data-ev-id="ev_0b5c55eaf1" className="flex-1 min-w-0">
                  ...
                  <div data-ev-id="ev_4cfe189bc7" className="text-white/25 text-[9px] font-mono truncate">{link.subtitle}</div>
                </div>
              ...
              style={{ color: STATUS_COLORS[status], background: STATUS_COLORS[status] + '15' }}>
                ...
                {/* Uptime */}
                <span data-ev-id="ev_35be3be966" className="text-white/30 text-[10px] font-mono w-12 text-right flex-shrink-0 hidden sm:block">
                  {fakeUptime(link.id)}%
                </span>

                {/* Metric */}
                <span data-ev-id="ev_ca305c20b3" className="text-white/40 text-[10px] font-mono w-14 text-right flex-shrink-0 hidden md:block">
                  {fakeMetric(link.id)}
                </span>
                ...
                {/* Link */}
                <ExternalLink className="w-3 h-3 text-white/10 group-hover:text-white/40 transition-colors flex-shrink-0" />
              </a>);
