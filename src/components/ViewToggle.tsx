...
const views: {id: number;label: string;icon: JSX.Element;gradient: string;}[] = [
{
  id: 1,
  label: 'Grid',
...
// Split views into rows of ROW_SIZE
const ROW_SIZE = 6;
const rows: typeof views[] = [];
      ...
      style={{ WebkitOverflowScrolling: 'touch' }}>
          ...
          return (
            ...
            'text-white/55 hover:text-white/80 hover:bg-white/[0.06]'}
              ...
              className={`absolute inset-0 rounded-lg sm:rounded-xl bg-gradient-to-r ${v.gradient} opacity-85`}
              aria-hidden="true" />
              ...
              <div data-ev-id="ev_0bc040931a" className="absolute inset-0 rounded-lg sm:rounded-xl border border-white/[0.08]" aria-hidden="true" />
                ...
                <span data-ev-id="ev_30982cdf31" className="relative z-10 flex items-center gap-1.5 sm:gap-1.5">
                  <span data-ev-id="ev_724362a487" className="flex-shrink-0" aria-hidden="true">{v.icon}</span>
                  <span data-ev-id="ev_575346d926" className="hidden min-[375px]:inline">{v.label}</span>
                </span>
              </button>);
